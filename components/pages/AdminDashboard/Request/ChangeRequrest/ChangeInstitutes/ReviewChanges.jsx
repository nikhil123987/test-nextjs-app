...
const ReviewChanges = () => {
  ...
  useEffect(() => {
    ...
    if (instituteDetails?.updatedRequest?.totalfaculties) {
      setTotalFaculties(instituteDetails?.updatedRequest?.totalfaculties);
    } else {
      setTotalFaculties(instituteDetails?.totalfaculties || instituteDetails?.faculties?.length);
    }
    ...
    if (instituteName) {
      ...
      const updatedData = {
        ...
        updates: {
          ...
          slug: `${titleToUrl(instituteDetails?.instituteName)}`,
        },
      };
      ...
      <div className="w-full">
        <section className=" w-full h-screen font-dm-sans overflow-x-hidden overflow-y-scroll relative space-y-4">
            ...
            <section className="  w-full px-5 space-y-4 lg:space-y-0 lg:px-12 mb-6">
              ...
              {isDropDown1 && (
                <Fragment>
                  ...
                  <CustomInputField
                      inputState={[totalFaculties, setTotalFaculties]}
                      description={totalFaculties}
                      className=" lg:w-96 shrink mb-4 lg:mb-0"
                      disableState={[isDisable, setIsDisable]}
          ...
          <div className="text-center">
            ...
            <button
              className="ml-3 bg-red/60 my-2 transition-all  hover:-translate-y-1 shadow-md hover:shadow-lg rounded px-16 py-3 text-white font-medium "
              onClick={() => {
                navigate("/admin-dashboard/requests/institute-requests");
                setIsDisable(true);
              }}
