...
const Toppers = () => {
  ...
  useEffect(() => {
    if (currentInstitute?.toppers?.length) {
      setData(currentInstitute?.toppers);
    }
  }, [currentInstitute]);

  return (
    <div>
      {currentInstitute?.toppers?.length ? (
        <>
          <div className="py-5 ">
            <AuthModal
            ...
            <p className="px-5 text-[35px] text-[#414141]  font-bold">
              {currentInstitute?.name} Toppers
            </p>
            <p className="px-5 md:w-3/4 my-5  text-[18px] text-[#888888]">
              Students bringing out the best in the All examination - Subscribe
              Now if you want to attend your dream college like hundreds of our
              students.
            </p>

            <div className={""}>
              <div className="md:max-w-[1400px] md:mx-auto md:py-5 px-1">
                <Carousel
